...
El colegio “TRENKE” aplica un sistema de evaluación particular a sus alumnos donde la 
evaluación se hace de forma constante en cada sesión de clase. Para ello, al finalizar 
cada sesión de clase el profesor tomará una evaluación de los temas vistos en dicha sesión 
y de una duración máxima de veinte minutos. Si el alumno faltara a dicha sesión su nota para 
esa evaluación será de cero (0).
...
a)	Desarrollar un subprograma que teniendo como base las 13 notas de un alumno, nos retorne 
el promedio final obtenido. El promedio final se calculará tomando las 10 mejores notas 
obtenidas durante las 13 sesiones que tiene el curso, no se toman las 3 menores.
b)	Desarrollar un subprograma reciba las notas promedio final de todos los alumnos y 
determine quién fue el mejor alumno de la clase.
c)	Desarrollar un subprograma que reciba las notas promedio final de los alumnos y determine 
el promedio de todo el salón.
=end
def calcularpromedioalumno(notas)
	...
	return ( total_notas / 10.00 ).round(2)
end

def determinarmejoralumno(promedios,alumnos)
 	notas_promedios = promedios[0]
	for i in 0...promedios.size 	
 		if notas_promedios < promedios[i]
 			notas_promedios = promedios[i]
 			mejor_alumno = i 
 		end
 	end
 	return alumnos[mejor_alumno]
end

def calcularpromediosalon(promedios)
 	total_promedios = 0.0
 	for i in 0...promedios.size
 		total_promedios = total_promedios + promedios[i]
 	end
 	return ( total_promedios / promedios.size ).round(2)
end
